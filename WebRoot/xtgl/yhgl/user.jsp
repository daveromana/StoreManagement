...
function userUpdate(){
	...
	if(count == 1)
		location.href="User!edit.do?id="+id;
	else
		alert("请选择其中一项进行修改");
}
function userDelete(){
	...
	if(count == 1){
		if(confirm("用户会被删除！ 确定要删除？"))
			location.href="User!datagrid.do?methodType=delete&id="+id;
	}
	else
		alert("请选择其中一项进行删除");
}
...
<body>
	...
	<br> 
	<form action="User!datagrid.do?methodType=query" method="post">
		用户姓名
		<input type="text" name="queryusername" value="${sessionScope.queryUserName }"> 
		<input type="submit" value="过滤">
		<input type="button" value="取消"
		onclick="location.href='User!datagrid.do'">
	</form>
	<input type="button" value="增加" onclick="location.href='User!add.do'">
	<input type="button" value="修改" onclick="userUpdate()">
	<input type="button" value="删除" onclick="userDelete()">
	<a href="User!datagrid.do?current_page=1">首页</a> &nbsp
	<a
		href="User!datagrid.do?current_page=<%=current_page > 1 ? current_page - 1 : current_page%>">上一页</a>
	&nbsp
	<a
		href="User!datagrid.do?current_page=<%=current_page < maxCount ? current_page + 1 : current_page%>">下一页</a>
	&nbsp
	<a href="User!datagrid.do?current_page=<%=maxCount%>">尾页</a>
