...
public class UserDatagridServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if ("delete".equals(methodType)) {
			...
			try {
				...
				response.sendRedirect("xtgl/yhgl/user.jsp?current_page="
						+ current_page);
	...
	private void query(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			//如果不为空
			if (!"".equals(userName)) {
				List userList = us.getAllUser(userName);
				request.getSession().setAttribute("userList", userList);
	...
	private void delete(HttpServletRequest request, HttpServletResponse response)
		...
		if (us.deleteOneUser(id)) {
			...
			response.sendRedirect("xtgl/yhgl/user.jsp?result=failed");
		}
	}
